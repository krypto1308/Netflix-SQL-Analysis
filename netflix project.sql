...
--2. Find the most common rating for movies and TV shows
	...
	final_rating as (select
   	type, rating ,totalcount,
	   rank() 
	   over(partition by type order by totalcount desc) as rank
	   from rating_count 
	)
select type, rating as ans_rating
from final_rating 
where rank=1;
